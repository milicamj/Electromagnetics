...
(DRAFT Milica Markovic Fall 2006)                 % text you want to see
true charpath
...
The total field due to both charges is
...
Here we will find the electric field at a point $P(x,y,z)$ due to a charge $q_1$ positioned at a point $P_{q_1}(x_1, y_1, z_1 )$  in the rectangular coordinate system. The position vector of the point $P_{q_1}$  is 
\begin{eqnarray}
...
\begin{eqnarray}
\vec{r_1}=x\vec{x} + y \vec{y} +z \vec{z}
\end{eqnarray}
...
\begin{eqnarray}
\vec{r_a}= (x_1 – x) \vec{x} +(y_1 – y) \vec{y} +(z_1 – z) \vec{z}
\end{eqnarray}
...
\begin{eqnarray}
|\vec{r_a}|= \sqrt((x_1 – x)^2 +(y_1 – y)^2 +(z_1 – z)^2)
\end{eqnarray}
...
\hat{r_a}=\frac{\vec{r_a}}{\sqrt((x_1 – x)^2 +(y_1 – y)^2 +(z_1 – z)^2)}
\end{eqnarray}
...
Where $r_a$ is the distance between the charge $q_1$ and the point $P$. Substituting expressions for $\hat{r_a}$, 
